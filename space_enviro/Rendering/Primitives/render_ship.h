...
void RenderShip(sf::VertexArray* varr, int offset, entity_data::Ship* ships, int i){
  ...
  varr->operator[](offset+1).position=sf::Vector2f(
      (1.f/3)*size*sinf(angle)+x - ((1.f/2)*sidepower*size*sinf(angle + M_PI_2f32) + (1.f / 4)), //FIXME magic number
      (1.f/3)*size*cosf(angle)+y - ((1.f/2)*sidepower*size*cosf(angle + M_PI_2f32) + (1.f / 4))); //FIXME magic number
